...
export default function Home() {
  ...
  const [claimBtnStatus, setClaimBtnStatus] = useState("active");
  const [claimReleaseBtnStatus, setClaimReleaseBtnStatus] = useState("active");
  ...
  const claimToken = () => {
    // setCanClaim(false);
    setClaimBtnStatus("claiming");
    console.log(0, claimAmount, blindAddress);
    IDO_CONTRACT.claim(0, String(claimAmount), blindAddress)
      .then((tx: any) => {
        tx.wait().then((res: any) => {
          console.log("tx res!", res.status);
          if (res.status === 1) {
  ...
  const claimRelaseToken = () => {
    if (parseInt(userReleaseInfo.currentClaimable)) {
      setClaimReleaseBtnStatus("claiming");
      LOCK_CONTRACT.ClaimToken()
        .then((tx: any) => {
          tx.wait().then((res: any) => {
            ...
            setClaimReleaseBtnStatus("active");
          });
        })
        .catch((err: any) => {
          console.log(err);
